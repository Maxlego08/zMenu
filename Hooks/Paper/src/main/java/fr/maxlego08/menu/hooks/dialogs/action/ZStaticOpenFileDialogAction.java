...
public class ZStaticOpenFileDialogAction implements ZDialogAction {
    private final @NotNull String filePath;

    public ZStaticOpenFileDialogAction(@NotNull String filePath) {
    ...
    public DialogAction build(@NotNull List<@NotNull DialogInput> inputs, @NotNull Player player, @NotNull MenuPlugin plugin, @NotNull InventoryEngine inventoryEngine, @Nullable DialogButton<?> button, @NotNull Placeholders placeholders) {
        return DialogAction.staticAction(ClickEvent.openFile(plugin.parse(player, placeholders.parse(this.filePath))));
    }
}
