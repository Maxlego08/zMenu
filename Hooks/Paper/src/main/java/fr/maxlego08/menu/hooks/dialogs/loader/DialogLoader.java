...
public class DialogLoader implements Loader<ZDialogs> {
    ...
    public ZDialogs load(YamlConfiguration configuration, String path, Object... objects) throws InventoryException {
        ...
        String externalTitle = configuration.getString("external_title", "");
        ...
        String afterActionString = configuration.getString("after_action", "CLOSE");
        try {
    ...
    private void loadSpecificItems(DialogType dialogType, YamlConfiguration configuration, ZDialogInventory dialogInventory, File file) {
        switch (dialogType) {
            ...
            case CONFIRMATION -> {
                ...
                String yesText = configuration.getString("yes.text");
                String noText = configuration.getString("no.text");
                int yesWidth = configuration.getInt("yes.width", 100);
                String yesTooltip = configuration.getString("yes.tooltip");
                String noTooltip = configuration.getString("no.tooltip");
                int noWidth = configuration.getInt("no.width", 100);
