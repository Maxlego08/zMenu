...
public class ActionClickLoader implements Loader<ActionClick> {
    ...
    @Override
    public ActionClick load(YamlConfiguration configuration, String path, Object... objects) throws InventoryException {

        Loader<ActionPlayerData> loader = new ActionPlayerDataLoader();

        boolean closeInventory = configuration.getBoolean(path + "closeInventory");
        ...
        return new ZActionClick(this.plugin, messages, playerCommands, consoleCommands, openLink, soundOption, actionPlayerDatas, closeInventory);
    }

    @Override
    public void save(ActionClick object, YamlConfiguration configuration, String path, File file, Object... objects) {

        configuration.set(path + "closeInventory", object.closeInventory());
        ...
        if (!object.getPlayerDatas().isEmpty()) {
            ActionPlayerDataLoader loader = new ActionPlayerDataLoader();

            for (ActionPlayerData playerData : object.getPlayerDatas()) {
                loader.save(playerData, configuration, path + "datas." + playerData.getKey() + ".", file);
            }
        }
